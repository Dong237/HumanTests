...
export function downloadJSON(result: TestResult): void {
  ...
  link.href = url;
  link.download = `onet-interest-profiler-results-${Date.now()}.json`;
  link.click();
...
export function downloadCSV(result: TestResult): void {
  ...
  const link = document.createElement('a');
  link.href = url;
  link.download = `onet-interest-profiler-results-${Date.now()}.csv`;
  link.click();
  URL.revokeObjectURL(url);
}
...
export async function downloadPDF(result: TestResult): Promise<void> {
  ...
  addText('O*NET Interest Profiler 职业兴趣测试报告', 20, true);
  yPosition += 10;
  ...
  topTypes.forEach((type) => {
    const typeConfig = scoringConfig.types[type.typeId as keyof typeof scoringConfig.types];
    addText(`${type.typeName} (${type.typeNameEn}) - ${type.rawScore}/40`, 12, true);
  ...
  result.typeScores.forEach((type) => {
    ...
    yPosition += 2;
    addText(`得分: ${type.rawScore}/40 | 百分比: ${type.percentage}% | 水平: ${getLevelLabel(type.level)}`, 10);
    yPosition += 2;
  ...
  pdf.setFontSize(8);
  pdf.text('本报告由O*NET Interest Profiler职业兴趣测试系统生成', pageWidth / 2, yPosition, { align: 'center' });

  pdf.save(`onet-interest-profiler-report-${Date.now()}.pdf`);
}

export async function copyToClipboard(result: TestResult): Promise<boolean> {
  let text = '=== O*NET Interest Profiler 职业兴趣测试结果 ===\n\n';
  ...
  topTypes.forEach((type) => {
    const typeConfig = scoringConfig.types[type.typeId as keyof typeof scoringConfig.types];
    text += `★ ${type.typeName} (${type.rawScore}/40)\n`;
  ...
  result.typeScores.forEach((type) => {
    ...
    text += `  得分: ${type.rawScore}/40 | 百分比: ${type.percentage}% | 水平: ${getLevelLabel(type.level)}\n`;
