...
import questionsData from '../../data/onet-questions.json';
...
const TestFlow: React.FC<TestFlowProps> = ({ onComplete }) => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('onet-progress');
    if (saved) {
  ...
  // Save progress
  useEffect(() => {
    localStorage.setItem('onet-progress', JSON.stringify({
      answers,
      index: currentIndex,
    }));
  ...
  const handleNext = () => {
    if (currentIndex < questions.length - 1) {
      ...
      // Complete the test
      localStorage.removeItem('onet-progress');
      onComplete(answers);
    }
  };
