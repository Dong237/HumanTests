      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="excalidraw-card mb-6">
          ...
          {expandedSections.has('types') && (
            <div className="p-6 pt-2 space-y-4">
              <p className="text-gray-700 leading-relaxed">{introData.riasecModel.description}</p>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
                {introData.riasecModel.types.map((type) => (
                  <div key={type.id} className="bg-white p-4 rounded-xl border-2 border-gray-300 hover:border-purple-400 transition-colors">
                            ...
                            content={
                              <div className="space-y-3">
                                ...
                                <div className="bg-purple-50 p-3 rounded-lg border border-purple-200">
                                  <p className="text-purple-800 font-medium">💼 适合职业：{type.example}</p>
                                </div>
                              </div>
                            }
                          />
        ...
        <div className="excalidraw-card mb-6">
          <button
            onClick={() => toggleSection('onetconnection')}
            className="w-full flex items-center justify-between p-4 hover:bg-gray-50 rounded-lg transition-colors"
          >
            <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-3">
              <span>🏷️</span>
              {introData.onetConnection.title}
            </h2>
            <span className="text-2xl transform transition-transform duration-200"
              style={{ transform: expandedSections.has('onetconnection') ? 'rotate(180deg)' : 'rotate(0deg)' }}>
              ▼
            </span>
          </button>
          {expandedSections.has('onetconnection') && (
            <div className="p-6 pt-2 space-y-4">
              <p className="text-gray-700 leading-relaxed">{introData.onetConnection.description}</p>
              <div className="bg-purple-50 p-4 rounded-xl border-2 border-purple-200">
                <h3 className="font-semibold text-purple-900 mb-3">职业信息内容</h3>
                <ul className="space-y-2">
                  {introData.onetConnection.features.map((feature, idx) => (
                    <li key={idx} className="text-gray-700 flex items-start gap-2">
                      <span className="text-purple-500 mt-1">✓</span>
                      <span>{feature}</span>
                    </li>
              ...
              <p className="text-gray-600 text-sm italic">{introData.onetConnection.note}</p>
            </div>
          )}
        </div>
          ...
          {expandedSections.has('howto') && (
            <div className="p-6 pt-2 space-y-4">
              {introData.howToTake.instructions.map((instruction) => (
                <div key={instruction.step} className="bg-gradient-to-r from-indigo-50 to-purple-50 p-4 rounded-xl border-2 border-indigo-200">
                  ...
                  <InfoIcon
                    ...
                    content={
                      <div className="space-y-2">
                        <p>5点评分量表是心理学研究中常用的自评工具，通过5个等级（0-4分）来衡量您对每个活动的喜好程度。</p>
                        <p>本测试中，0分代表"非常不喜欢"，4分代表"非常喜欢"，帮助您精确评估各项活动的兴趣水平。</p>
                      </div>
                    }
                  />
                </h3>
                <p className="text-indigo-800 mb-4">{introData.howToTake.likertScale.description}</p>
                <div className="space-y-2">
                  {introData.howToTake.likertScale.options.map((option) => (
                    <div key={option.value} className="flex items-center gap-3 bg-white p-2 rounded-lg border border-indigo-200">
                      <div className="w-8 h-8 rounded-full bg-indigo-200 flex items-center justify-center font-bold text-indigo-900">
                        {option.value}
                      </div>
                      <div className="flex-1">
            ...
            <div className="p-6 pt-2">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {introData.whatToExpect.sections.map((section, idx) => (
                  <div key={idx} className="bg-gradient-to-br from-indigo-50 to-purple-50 p-4 rounded-xl border-2 border-indigo-200">
        ...
        <div className="excalidraw-card mb-6">
          <div className="p-6">
            ...
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 p-4 rounded-xl border-2 border-indigo-200 mb-6">
