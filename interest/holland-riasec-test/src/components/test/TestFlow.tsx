...
import questionsData from '../../data/holland-questions.json';
...
const TestFlow: React.FC<TestFlowProps> = ({ onComplete }) => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('holland-progress');
    if (saved) {
  ...
  // Save progress
  useEffect(() => {
    localStorage.setItem('holland-progress', JSON.stringify({
      answers,
      index: currentIndex,
    }));
