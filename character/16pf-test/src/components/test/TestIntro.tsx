...
const TestIntro: React.FC<TestIntroProps> = ({ onStart, introData }) => {
      ...
      <div className="max-w-4xl mx-auto">
            ...
            <div className="p-6 pt-2 space-y-4">
              ...
              <div className="bg-teal-50 p-4 rounded-xl border-2 border-teal-200">
                ...
                <ul className="space-y-2">
                  {introData.overview.features.map((feature: string, idx: number) => (
        ...
        <div className="excalidraw-card mb-6">
          ...
          >
            <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-3">
              <span>🧠</span>
              {introData.factorModel.title}
            </h2>
            <span className="text-2xl transform transition-transform duration-200"
              style={{ transform: expandedSections.has('factors') ? 'rotate(180deg)' : 'rotate(0deg)' }}>
              ▼
            </span>
          </button>
          {expandedSections.has('factors') && (
            <div className="p-6 pt-2 space-y-4">
              <p className="text-gray-700 leading-relaxed">{introData.factorModel.description}</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-6">
                {introData.factorModel.factors.map((factor: any) => (
                  <div key={factor.id} className="bg-white p-3 rounded-xl border-2 border-gray-300 hover:border-teal-400 transition-colors">
                    <div className="flex items-start gap-2">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="font-bold text-teal-700">{factor.id}</span>
                          <h3 className="text-base font-bold text-gray-800">{factor.name}</h3>
                          <InfoIcon
                            title={`${factor.id} - ${factor.name}`}
                            content={
                              <div className="space-y-3">
                                <p className="text-gray-700">{factor.description}</p>
                                <div className="bg-teal-50 p-3 rounded-lg border border-teal-200">
                                  <p className="text-teal-800 text-sm">
                                    <strong>低分:</strong> {factor.lowPole} ←→ <strong>高分:</strong> {factor.highPole}
                                  </p>
                                </div>
                              </div>
                            }
                        ...
                        <p className="text-gray-600 text-sm">{factor.lowPole} ↔ {factor.highPole}</p>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {/* Sten Scoring Explanation */}
        <div className="excalidraw-card mb-6">
          <button
            onClick={() => toggleSection('sten')}
            className="w-full flex items-center justify-between p-4 hover:bg-gray-50 rounded-lg transition-colors"
          >
            <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-3">
              <span>📊</span>
              {introData.stenScoring.title}
            </h2>
            <span className="text-2xl transform transition-transform duration-200"
              style={{ transform: expandedSections.has('sten') ? 'rotate(180deg)' : 'rotate(0deg)' }}>
              ▼
            </span>
          </button>
          {expandedSections.has('sten') && (
            <div className="p-6 pt-2 space-y-4">
              <p className="text-gray-700 leading-relaxed">{introData.stenScoring.description}</p>
              <div className="bg-cyan-50 p-4 rounded-xl border-2 border-cyan-200">
                <h3 className="font-semibold text-cyan-900 mb-3">Sten分数范围</h3>
                <div className="space-y-2">
                  {introData.stenScoring.ranges.map((range: any, idx: number) => (
                    <div key={idx} className="flex items-center gap-3 bg-white p-2 rounded-lg border border-cyan-200">
                      <div className="w-16 text-center font-bold text-gray-800">
                        {range.range}
                      </div>
                      <div className="flex-1">
                        <span className="font-semibold text-gray-800">{range.label}</span>
                        <span className="text-gray-600 text-sm ml-2">— {range.description}</span>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="excalidraw-card mb-6">
          ...
          {expandedSections.has('howto') && (
            <div className="p-6 pt-2 space-y-4">
              {introData.howToTake.instructions.map((instruction: any) => (
                <div key={instruction.step} className="bg-gradient-to-r from-teal-50 to-cyan-50 p-4 rounded-xl border-2 border-teal-200">
              ...
              {/* Likert Scale */}
              <div className="mt-6 bg-teal-50 p-5 rounded-xl border-2 border-teal-200">
                <h3 className="font-bold text-teal-900 mb-3 flex items-center gap-2">
                  ...
                  <InfoIcon
                    ...
                    content={
                      <div className="space-y-2">
                        <p>Likert量表是心理学研究中常用的自评工具，通过5个等级来衡量您对每个描述的同意程度。</p>
                        <p>本测试中，5点量表从"非常不同意"到"非常同意"，帮助您精确评估各项人格倾向。</p>
                      </div>
                    }
                  />
                </h3>
                <p className="text-teal-800 mb-4">{introData.howToTake.likertScale.description}</p>
                <div className="space-y-2">
                  {introData.howToTake.likertScale.options.map((option: any) => (
                    <div key={option.value} className="flex items-center gap-3 bg-white p-2 rounded-lg border border-teal-200">
                      <div className="w-8 h-8 rounded-full bg-teal-200 flex items-center justify-center font-bold text-teal-900">
                        {option.value}
                      </div>
                      <div className="flex-1">
          ...
          {expandedSections.has('expect') && (
            <div className="p-6 pt-2">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {introData.whatToExpect.sections.map((section: any, idx: number) => (
                  <div key={idx} className="bg-gradient-to-br from-teal-50 to-cyan-50 p-4 rounded-xl border-2 border-teal-200">
          ...
          <div className="p-6 space-y-4">
            <div className="bg-green-50 p-4 rounded-xl border-2 border-green-200">
              ...
              <ul className="space-y-1">
                {introData.privacy.assurances.map((assurance: string, idx: number) => (
            ...
            <div className="bg-orange-50 p-4 rounded-xl border-2 border-orange-200">
              ...
              <div className="space-y-2">
                {introData.disclaimer.points.map((point: any, idx: number) => (
        ...
        <div className="excalidraw-card mb-6">
          <div className="p-6">
            ...
            <div className="bg-blue-50 p-5 rounded-xl border-2 border-blue-200 mb-6">
              ...
              <div className="space-y-2">
                {introData.readyToStart.checklist.map((item: string, idx: number) => (
            ...
            <div className="bg-gradient-to-r from-teal-50 to-cyan-50 p-4 rounded-xl border-2 border-teal-200 mb-6">
            ...
            <div className="text-center">
              <button
                onClick={onStart}
                className="excalidraw-button text-lg px-12 py-4 text-white bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 cursor-pointer transition-all transform hover:scale-105"
              >
          ...
          {expandedSections.has('science') && (
            <div className="p-6 pt-2 space-y-3 text-sm text-gray-600">
              ...
              <div className="bg-gray-50 p-3 rounded-lg">
                ...
                <ul className="space-y-1">
                  {introData.scientificBasis.applications.map((app: string, idx: number) => (
              ...
              <div className="bg-gray-50 p-3 rounded-lg">
                ...
                {introData.scientificBasis.references.map((ref: string, idx: number) => (
