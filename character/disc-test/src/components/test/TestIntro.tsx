    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50 py-12 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="excalidraw-card mb-8 text-center">
          ...
          <p className="text-lg text-amber-600 italic">{introData.welcome.tagline}</p>
        </div>
          ...
          {expandedSections.has('overview') && (
            <div className="p-6 pt-2 space-y-4">
              ...
              <div className="bg-amber-50 p-4 rounded-xl border-2 border-amber-200">
                <h3 className="font-semibold text-amber-900 mb-3">核心特点</h3>
        ...
        <div className="excalidraw-card mb-6">
          ...
          {expandedSections.has('types') && (
            <div className="p-6 pt-2 space-y-4">
              <p className="text-gray-700 leading-relaxed">{introData.discModel.description}</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                {introData.discModel.types.map((type) => (
                  <div key={type.id} className="bg-white p-4 rounded-xl border-2 border-gray-300 hover:border-amber-400 transition-colors">
                            ...
                            content={
                              <div className="space-y-3">
                                ...
                                <div className="bg-amber-50 p-3 rounded-lg border border-amber-200">
                                  <p className="text-amber-800 font-medium">💼 适合职业：{type.example}</p>
                                </div>
                              </div>
                            }
                          />
        ...
        {/* DISC Quadrant Explanation */}
        <div className="excalidraw-card mb-6">
          <button
            onClick={() => toggleSection('quadrant')}
            className="w-full flex items-center justify-between p-4 hover:bg-gray-50 rounded-lg transition-colors"
          >
            <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-3">
              <span>🏷️</span>
              {introData.discQuadrant.title}
            </h2>
            <span className="text-2xl transform transition-transform duration-200"
              style={{ transform: expandedSections.has('quadrant') ? 'rotate(180deg)' : 'rotate(0deg)' }}>
              ▼
            </span>
          </button>
          {expandedSections.has('quadrant') && (
            <div className="p-6 pt-2 space-y-4">
              <p className="text-gray-700 leading-relaxed">{introData.discQuadrant.description}</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
                {introData.discQuadrant.quadrants.map((quadrant, idx) => (
                  <div key={idx} className="bg-gradient-to-r from-amber-50 to-orange-50 p-3 rounded-xl border-2 border-amber-200">
                    <span className="font-bold text-amber-800 text-lg">{quadrant.type}</span>
                    <span className="text-gray-600 text-sm ml-3">{quadrant.keywords}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {expandedSections.has('howto') && (
            <div className="p-6 pt-2 space-y-4">
              {introData.howToTake.instructions.map((instruction) => (
                <div key={instruction.step} className="bg-gradient-to-r from-orange-50 to-amber-50 p-4 rounded-xl border-2 border-orange-200">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-amber-700 text-sm italic">💡 {instruction.tip}</p>
                    </div>
                  </div>
                </div>
              ))}
                  ...
                  <InfoIcon
                    ...
                    content={
                      <div className="space-y-2">
                        ...
                        <p>本测试中，5点量表从"非常不符合"到"非常符合"，帮助您精确评估各项行为倾向。</p>
                      </div>
                    }
                  />
                </h3>
            ...
            <div className="p-6 pt-2">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {introData.whatToExpect.sections.map((section, idx) => (
                  <div key={idx} className="bg-gradient-to-br from-orange-50 to-amber-50 p-4 rounded-xl border-2 border-orange-200">
        ...
        <div className="excalidraw-card mb-6">
          <div className="p-6">
            ...
            <div className="bg-gradient-to-r from-orange-50 to-amber-50 p-4 rounded-xl border-2 border-orange-200 mb-6">
            ...
            <div className="text-center">
              <button
                onClick={onStart}
                className="excalidraw-button text-lg px-12 py-4 text-white bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 cursor-pointer transition-all transform hover:scale-105"
              >
